...
use IPC::Open2;                 # CaboCha をシェル経由せず呼ぶため
...
#    $pStatement =~ s/,\s?/、/g;
    ...
    for ($pLine=0; $pLine<=$#gaStatement; $pLine++) {
        #CaboCha実行
        # 以前は `echo $文 | cabocha` とシェル経由で呼んでおり、入力に含まれる
        # バッククォート等でコマンドインジェクションの余地があった（CWE-78）。
        # シェルを介さず open2 で cabocha を起動し、文は標準入力から渡す。
        @paCaboCha=();
        {
            my $pCbPid=open2(my $pCbOut, my $pCbIn, $cpCABOCHA, '-f1');
            print $pCbIn $gaStatement[$pLine], "\n";
            close $pCbIn;
            @paCaboCha=<$pCbOut>;
            close $pCbOut;
            waitpid($pCbPid, 0);
